...
`termux-tools` is a separate configure-time path boundary. Its upstream `configure.ac` intentionally defaults the generated interactive-login/profile scripts to `com.termux` when `TERMUX_APP_PACKAGE`, base/cache, prefix, or home variables are absent. Copper's deterministic patch exports all of those build values before that configure step. A completed bootstrap additionally scans its executable/configuration text members for legacy `/data/data/com.termux` roots and unresolved `@TERMUX_*@` placeholders, then asserts Copper-path fragments in the profile initializer, the bootstrap second-stage entry point, and its fallback launcher. The build fails before staging/publishing if any one of those checks fails; this is an archive-build gate, not a claim that an x86 emulator has accepted the arm64 runtime on a physical device.
