...
- [ ] Run the opt-in `[runtime-asset-validation]` CI gate against that exact temporary artifact. The gate checks the GitHub enclosing-artifact identity, validates the contained ZIP against its JSON manifest, materializes both only in the runner's Android module assets, then uses API-35's ARM native-bridge emulator to exercise the atomic installer and a Copper Bash PTY command. Its instrumentation target is explicitly `com.copper.chat`, since the bootstrap prefix is compiled for that application identity. It is not release packaging, and its locked temporary artifact must be renewed before expiration.
